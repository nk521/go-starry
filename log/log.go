...
func NewLogger(cfg *config.StarryConfig) *logrus.Logger {
	return newLogrusLogger(cfg)
}

func newLogrusLogger(cfg *config.StarryConfig) *logrus.Logger {

	l := logrus.New()

	if cfg.Meta.JSONLogs {
		l.Formatter = new(logrus.JSONFormatter)
	}
	l.Out = os.Stderr

	switch cfg.Meta.LogLevel {
	case "debug":
